...
- Built a template with `ready` status following [OpenClaw Template](../04.Features/02.Templates/07.Other%20Templates/05.OpenClaw%20Template.md) and noted the template name (referred to as `TEMPLATE_NAME` below)
...
# Register the OpenAI-compatible provider. The model id must match the
# provider's actual model name; an unregistered bare openai/<model> fails with
# "Unknown model".
...
The image does not preload custom Skills. For Skill scopes, directory conventions, and trigger behavior, see [OpenClaw Template](../04.Features/02.Templates/07.Other%20Templates/05.OpenClaw%20Template.md). For the complete mechanism, see [OpenClaw Skills](https://docs.openclaw.ai/tools/skills).
...
The sandbox has public outbound access by default, so you can search and install community Skills directly (browse [clawhub.ai](https://clawhub.ai)). Obtain a Skill slug from the search results before installing it.
...
# Install a Skill returned by the search
result = sandbox.commands.run("openclaw skills install ws-calendar", timeout=180)
print(result.stdout)
```

### Browser Access (FC E2B)

When `sandbox.get_host(PORT)` returns the default platform domain (`*.e2b.fc.aliyuncs.com`), `curl` or programmatic clients can access the Gateway with the `X-Access-Token` header. This domain returns the Control UI page as an attachment, so a browser downloads the page instead of rendering it even when an extension adds `X-Access-Token`. To open the Control UI in a browser, you must bind a custom domain. For the complete steps on adding a domain in the console, DNS resolution, HTTPS certificates, and SDK configuration, see [Cloud Sandbox Custom Domains](../04.Features/07.FC%20Extensions/03.Custom%20Domains.md).

#### Verify the Gateway

When verifying the Gateway through the default platform domain, public access requires two layers of authentication:
...
Run the script above, note the `Gateway` URL and `Access Token`, and verify with `curl`. The request must include `X-Access-Token`; otherwise, FC E2B returns 403:
...
The expected response should include `200`. Receiving HTML indicates that the Gateway is running, but does not mean that the default platform domain can render in a browser.

#### Open the Control UI in a Browser

Bind a custom domain in the Function Compute console, configure certificates and DNS (`api.<your-custom-domain>` / `*.<your-custom-domain>`), then point the SDK environment variables at it before creating the sandbox. `E2B_API_KEY` must belong to the **same account that bound the custom domain**:
...
The SDK reads these variables automatically across `Template.build(...)`, `Sandbox.create(...)`, and all other calls, so you do not need to pass connection parameters into method calls. After completing the configuration:

1. Run the script in [Deploy the Gateway](#deploy-gateway) again and confirm that `sandbox.get_host(PORT)` returns `{PORT}-sbx-{sandbox_id}.<your-custom-domain>`.
2. Confirm that the Gateway's `allowedOrigins` exactly matches the browser origin, for example `https://{PORT}-sbx-{sandbox_id}.<your-custom-domain>`, with no trailing `/`.
3. Open the Gateway URL printed by the script in a browser. The URL already contains the Gateway Token in the `?token=` parameter.

If you see a "browser origin not allowed" error, correct `allowedOrigins` and [restart the Gateway](#restart-gateway).
...
For the Gateway default port and startup parameters, see [OpenClaw Template](../04.Features/02.Templates/07.Other%20Templates/05.OpenClaw%20Template.md).
...
# Register the provider. A bare openai/<model> that was never onboarded fails
# with "Unknown model".
sandbox.commands.run(
    "openclaw onboard --non-interactive --accept-risk --skip-health "
    "--auth-choice custom-api-key "
...
| Issue | Solution |
|------|------|
| Agent not responding | Verify that `envs` includes the model Key and that `onboard` has registered the provider |
| Gateway not opening in browser | Bind a custom domain to render the Control UI and ensure the URL contains `?token=`. Use the default platform domain only for `curl` or programmatic access with `X-Access-Token` |
...
| name 'TEMPLATE_NAME' is not defined | Replace `TEMPLATE_NAME` with the template name from [OpenClaw Template](../04.Features/02.Templates/07.Other%20Templates/05.OpenClaw%20Template.md) |
...
- [OpenClaw Template](../04.Features/02.Templates/07.Other%20Templates/05.OpenClaw%20Template.md) (image addresses, build and minimal verification, environment overview)
