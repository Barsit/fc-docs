...
Start the OpenClaw Gateway to interact with the Agent in a browser. The complete workflow below covers: **Create Sandbox → Configure → Start Gateway → Get Access URL**.

The following defaults to **Bailian** (recommended for China); international users can uncomment the OpenAI option in the comments and comment out the Bailian section. For server-side programmatic invocation, see [Agent CLI](#agent-cli).
...
sandbox = Sandbox.create(
    ...
    # International OpenAI: comment out the envs above and use the following instead
    # envs={"OPENAI_API_KEY": "<your-openai-api-key>"},
    **OPTS,
)

# Bailian: register provider (skip this step for OpenAI)
