...
| Gateway Web UI | Supports `openclaw gateway` to start a Web UI. The default platform domain supports `curl` or programmatic clients with `X-Access-Token`; rendering the Control UI in a browser requires a custom domain |
...
## Gateway and Skill conventions

The Gateway listens on port `18789` by default. Common startup parameters are:
...
Call a Skill explicitly with `openclaw agent ... --message "/skill-name"`, or let the model load it automatically when natural language matches its `description`. Place the file at `<workspace>/skills/<name>/SKILL.md`; a `SKILL.md` directly under the workspace root is not recognized. For the complete mechanism, see [OpenClaw Skills](https://docs.openclaw.ai/tools/skills). For real-task integration examples, see [Use OpenClaw Sandbox](../../../05.Best%20Practices/06.Use%20OpenClaw%20Sandbox.md).
...
Obtain an API Key, Base URL, and model name from your OpenAI-compatible provider; the example below uses DashScope OpenAI-compatible mode.
...
| Model Key | Not preconfigured in the image; must be injected at runtime via `envs`. OpenAI-compatible flows require `openclaw onboard` to register the provider first; an unregistered bare `openai/<model>` fails with "Unknown model" |
| Gateway access | Use the default platform domain only for `curl` or programmatic verification, with `X-Access-Token`. Opening it in a browser downloads the page instead of rendering it; bind a custom domain to render the Control UI |
