...
> The SDK and the API use different field naming styles: the SDK uses lower camelCase (e.g. `allowInternetAccess`, `allowPublicTraffic`), while the HTTP API uses snake_case (e.g. `allow_internet_access`, `network.allowPublicTraffic`). The examples below follow the SDK style.
...
export E2B_API_URL="http://api.<region>.e2b.fc.aliyuncs.com"
...
try {
  // Grab the token right after creation and save it yourself (it cannot be fetched again later; see below)
...
Recommendation: persist `trafficAccessToken` together with `sandboxId` for easy reuse; even if it is lost, you can retrieve it again through `connect`.
...
- `false`: forbids all egress, equivalent to setting `denyOut: ["0.0.0.0/0"]`.
...
2. **`allowInternetAccess: false` is equivalent to `denyOut: ["0.0.0.0/0"]`**: i.e. "deny all by default", then combine with `allowOut` to build an allowlist.
...
| `allowInternetAccess: false` | Forbids all egress |
