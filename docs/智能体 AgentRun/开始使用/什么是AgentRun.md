...
- 框架开放、不被锁定：
  ...
  - 提供适配主流 Agent 框架的集成能力，可与 LangChain、AgentScope、CrewAI、Google ADK 等框架结合使用（详见开发文档：[AgentRun SDK 说明](https://docs.agent.run/docs/tutorial/quickstart/manual)）；
...
- MCP 与 Function Call 双协议：
  ...
  - 支持 MCP 打包工具：将多个 MCP 工具
                        
                          和API工具
                        聚合为一个统一的 MCP 网关，通过单一端点对外提供服务，便于统一管理和访问。
...
![image](../../../assets/2998769-001-6eca23d538.png)
...
- 登录控制台：[AgentRun控制台](https://functionai.console.aliyun.com/cn-hangzhou/agent/runtime/agent-list)；
- 系统将自动检测是否已完成 SLR 授权。如未授权，会弹出授权指引对话框。按提示操作即可完成以下角色的创建和授权：
  
  - 自定义角色**AliyunDevsCustomRole**（信任实体：`devs.aliyuncs.com`）
  - 默认角色**AliyunDevsDefaultRole**
  - 服务关联角色**AliyunServiceRoleForFC**和**AliyunServiceRoleForAgentRun**
- 授权完成后即可正常创建和管理 Agent、Sandbox、模型等资源。
...
- 如需在 VPC 内网访问 AgentRun 服务，可通过 PrivateLink 配置内网访问端点，详见[通过PrivateLink内网访问AgentRun资源](https://help.aliyun.com/zh/functioncompute/fc/access-agenrun-resources-through-privatelink-intranet)；
