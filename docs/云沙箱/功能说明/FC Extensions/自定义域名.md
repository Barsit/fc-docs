...
|------------|----------|----------|------------|
| 控制链路 CNAME | `api` | CNAME | `api.cn-hangzhou.e2b.fc.aliyuncs.com` |
| 数据链路 CNAME | `*` | CNAME | `{AccountID}.cn-hangzhou.e2b.fc.aliyuncs.com` |

示例：如果您的主账号 ID 为 `119085698835****`，云沙箱所在地域为华东 1（杭州），数据链路 CNAME 可配置为：

```text
119085698835****.cn-hangzhou.e2b.fc.aliyuncs.com
```

如图所示：

![配置域名解析](./assets/custom-domain-dns.png)
