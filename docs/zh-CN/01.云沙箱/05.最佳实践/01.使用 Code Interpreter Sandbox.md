...
Code Interpreter Sandbox 可以通过两类 SDK 接入：

| SDK | 模板参数 | 适合场景 |
| --- | --- | --- |
| `e2b_code_interpreter` SDK | 不需要指定模板 | 需要 `run_code` 体验、交互式代码执行、Notebook 类 Agent |
| `e2b` SDK | 需要指定 `code-interpreter-v1` | 需要同时使用文件、命令、进程和模板能力的工程化任务 |

如果业务侧已经使用 `e2b_code_interpreter` SDK，可以直接创建 Sandbox 并调用 `run_code`。如果使用通用 `e2b` SDK，则需要显式选择 `code-interpreter-v1` 模板。
...
- [Code Interpreter 概览](../04.功能说明/05.代码解释器/01.概览.md)
