...
API Key 来自函数计算控制台的云沙箱 API Keys 管理页面。创建前需要已开通函数计算服务和云沙箱功能。进入控制台后，在**云沙箱** > **API Keys** 页面单击**创建 API Key**，按用途填写描述并设置过期时间。创建成功后复制完整 API Key 并妥善保管。详细操作参见[管理 API Key](https://help.aliyun.com/zh/functioncompute/management-api-key)。
...
API Key 应在函数计算控制台创建、查看、编辑、重置、禁用和删除，不通过 E2B SDK 管理。已禁用或已过期的 API Key 不能通过数据面认证。不要在日志、截图、工单或前端页面中暴露完整 API Key。
...
更多模板构建要求参见[自定义模板](../04.功能说明/02.Templates/03.自定义模板.md)。
