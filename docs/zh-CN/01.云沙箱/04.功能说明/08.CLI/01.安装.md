...
E2B CLI 使用 `E2B_API_KEY` 完成认证。接入云沙箱时，还需要配置 API URL 和 Domain：

API Key 来自函数计算控制台的云沙箱 API Keys 管理页面。创建方式参见[管理 API Key](https://help.aliyun.com/zh/functioncompute/management-api-key)。
...
- API Key 是否来自函数计算控制台的云沙箱 API Keys 管理页面。
