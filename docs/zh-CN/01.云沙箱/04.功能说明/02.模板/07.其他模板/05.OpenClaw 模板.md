...
| Gateway Web UI | 支持 `openclaw gateway` 启动 Web UI；默认平台域名可供 `curl` 或编程客户端携带 `X-Access-Token` 访问，在浏览器中渲染 Control UI 需绑定自定义域名 |
...
| Gateway 访问 | 默认平台域名仅用于 `curl` 或编程客户端验证，请求须带 `X-Access-Token`；浏览器打开默认平台域名会下载页面文件，渲染 Control UI 需绑定自定义域名 |
