...
删除后再次调用 `DescribeQuota` 查询同一个 `TagValue`，会返回 404 / `ResourceQuotaNotFound`。

### 错误处理

Go SDK 的接口错误统一为 `*tea.SDKError`，可通过类型断言读取 `Code`、`StatusCode` 和 `Message`。
...
注意：该示例会删除指定 Team 的配额。生产 Team 请去掉删除和确认删除步骤，或使用测试 Team 验证。
...
func main() {
	...
	if err != nil {
		...
		log.Fatal("[6] expected 404 but describe succeeded")
	}
...
删除后再查询同一个 `TagValue` 会返回 404 / `ResourceQuotaNotFound`。
