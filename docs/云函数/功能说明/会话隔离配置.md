...
| **会话存储隔离（动态挂载存储）功能** | 内测中，使用前请[提交工单](https://selfservice.console.aliyun.com/ticket/createIndex)申请； |
...
以下操作仅对会话隔离相关配置进行了详细说明，如对其他配置有疑问，可以查看[创建函数](https://help.aliyun.com/zh/functioncompute/fc/user-guide/function-instance-1/)相关文档；
...
4. **高级配置**>**隔离性、亲和性**：
  ...
  1. **实例隔离**：开启**实例隔离**，并选择**会话隔离**，开启会话隔离的同时，系统自动开启会话亲和，您需要进一步配置会话亲和相关参数。
  ...
  2. **会话亲和**：不同亲和类型配置参考[实例隔离配置实践](https://help.aliyun.com/zh/functioncompute/fc/instance-isolation-configuration-practices)，本文以**HeaderField 亲和**为例，配置**Header Name**，例如`x-affinity-header-v1`，用于传递亲和值客户端标识的头部名称，其余配置保持默认或按需配置；
...
1. 以配置HeaderField亲和为例，指定不同HTTP请求头的值调用函数进行测试
  ...
  **请求说明：**
  ...
  1. `x-affinity-header-v1`：为**会话亲和**配置的**Header Name**，根据实际配置进行替换；
...
2. 结果验证：
  ...
  在函数计算调用日志中，同一会话（Session1）的多次请求复用相同的实例 ID（如`c-687e0412-12f6c06f-df`），而不同会话（Session2）的请求被分配到不同的实例（如`c-687e03d5-12f6c06f-b`），验证了会话隔离配置已生效。
...
![image](../../../assets/3001390-001-5c22d5a92d.png)
...
- [配置网络](https://help.aliyun.com/zh/functioncompute/fc/user-guide/configure-network-settings)
